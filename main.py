...
from model import VIT, PatchEmbedding, multiHeadAttention, residual, mlp, TransformerBlock, Transformer, \
    Classification
import helper
from helper import ImgAugTransform, predict_on_video
...
Jedi = Blueprint('Jedi',__name__, static_folder= 'static',template_folder='templates')
...
## For resizing of any input size
...
@Jedi.route('/model_updates', methods=['GET', 'POST'])
def models_update():
    global model_path
...
model_4 = torch.load(os.path.join("Trained_Models","Fake_Face_Detection_Model_1.pth"), map_location=device)
...
model_7 = torch.load(os.path.join("Trained_Models", "Deepfake_Model_2.pth"), map_location=device)
model_7.to(device)
model_7.eval()
...
@Jedi.route('/twitteroutput', methods =["GET", "POST"])
def twitteroutput():
    if request.method == "POST":
       ...
       for i in df["image_url"]:
        ...
        label, score, fake_image_path = Fake_Face(image_filename, model_path)
...
@Jedi.route('/uploader1', methods=['GET', 'POST'])
def file_uploader():
    if request.method == 'POST':
        ...
        if video_url != "" and util.valid_youtube_video_url(video_url):
            ...
            if file_type == "video":
                ...
                print("Check here file path 2345")
                ...
                if label == -1:
                    ...
                    label, score, fake_image_path = Fake_Face(filename, model_path)
                    print("Check here file path 6789")
                    print(fake_image_path)
...
@Jedi.route('/predict_FIM', methods=['GET', 'POST'])
def predict_3():
    """Fake Image routing"""
    if request.method == 'POST':
        # check if the post request has the file part
...
@Jedi.route("/FIM_Pata_Karo/<filename>", methods=['GET'])
...
@Jedi.route('/predict_FFM', methods=['GET', 'POST'])
def predict_4():
    print("here")
    if request.method == 'POST':
        # check if the post request has the file part
...
# Fake image
@Jedi.route("/FFM_Pata_Karo/<filename>", methods=['GET'])
def detect_FFM(filename):
    final_image = os.path.join(UPLOAD_FACE_FOLDER, filename)
    label,score, fake_image_path = Fake_Face(filename, model_path)
    print("Check here file path")
    print(fake_image_path)
    a,b,c,d,e,f,g,h,i,j,k = metadata_image(filename)
    print("Modelaa",a)
    if label == -1:
...
@Jedi.route('/uploadfile', methods=['GET', 'POST'])
def upload_file():
    if request.method == 'POST':
        # check if the post request has the file part
        if 'file' not in request.files:
...
@Jedi.route('/return-files/<filename>')
